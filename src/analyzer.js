...
app.post('/ingest', async (req, res) => {
  try {
    ...
    for (const innerBytes of batch) {
      ...
      // wrappedAes is already decrypted? Not yet — above rsaDecrypt output IS the innerPackage, not AES key.
...
app.post('/ingest', async (req, res) => {
  try {
    ...
    for (const jsonBytes of batch) {
      let text = Buffer.from(jsonBytes).toString('utf8').trim();
      if (!text) continue;  // 跳過空字串
      try {
        pkg = JSON.parse(text);
      } catch (e) {
        console.warn("Skip invalid inner package:", text.slice(0, 50));
        continue;
      }
      ...
      const jwkJson = rsaDecrypt(AN_PRIV, Buffer.from(wrappedAes)).toString('utf8');
      const jwk = JSON.parse(jwkJson);


      // Decrypt AES-GCM payload
      const plainBuf = await aesDecryptGcm(jwk, iv, ct);
